...
void store(ll edge, vector<ll> v[], map< pair<ll,ll>, ll > &m)
{
    for(ll i=0; i<edge; i++) {
        ll a, b, c;
        cin >> a >> b >> c;
        v[a].push_back(b), m[ make_pair(a,b) ] = c;
        v[b].push_back(a), m[ make_pair(b,a) ] = c;// for undirected graph only
    }
}
...
ll price(vector<pair<ll,ll>> road, map< pair<ll,ll>, ll > m)
{
    ll cost = 0;
    for(ll j = 0; j < road.size(); j++)
        cost += m[ make_pair(road[j].first, road[j].second) ];
    return cost;
}
...
vector<pair<ll,ll>> Edge(ll start, ll edge, vector<ll> v[], vector<bool> &visit )
{
...
void SCC(ll vertex, ll edge, vector<ll> v[], vector<vector<ll>> &sccPoint, vector<vector<pair<ll,ll>>> &sccEdge, map< pair<ll,ll>, ll > m)
{
    vector<ll> omg;
    ...
    for(ll i = 0; i < omg.size(); i++) {
        ...
        if( visitEdge[ omg[i] ] == false ) {
            vector<pair<ll,ll>> omgEdge = Edge( omg[i], edge, u, visitEdge );
            if( omgEdge.empty() )
...
void printEdge(vector<vector<pair<ll,ll>>> scc, map< pair<ll,ll>, ll > m)
{
    for(ll i = 0; i < scc.size(); i++) {
        vector<pair<ll,ll>> road = scc[i];
        if( road.size()==1 && road[0].first==road[0].second )
            cout << road[0].first << " and cost " << 0 << lne;
        else {
            for(ll j = 0; j < road.size(); j++)
                cout << road[j].first << "-" << road[j].second << spc;
            cout << "and cost " << price(road, m) << lne;
        }
    }
    cout << lne;
}
...
//    inout
    ...
    SCC( vertex, edge, v, sccPoint, sccEdge, m ); /*can not properly print the edge of a negative cycle for an undirected graph*/
    printPoint(sccPoint);
    printEdge(sccEdge, m);
