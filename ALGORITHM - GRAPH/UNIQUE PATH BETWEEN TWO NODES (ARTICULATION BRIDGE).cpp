...
//CAN IDENTIFY THE UNIQUE PATH BETWEEN TWO NODES
...
void _Articulation(ll node, vector<ll> &parent, vector<ll> &discovery, vector<ll> &low, vector<ll> &path, vector<ll> v[])
{
    ll child = 0;
    static ll flag = 0;
    ...
    for(ll i=0; i<v[node].size(); i++) {
        ...
        if( discovery[x]==-1 ) { // if x is not visited yet, then make it a child of node in DFS tree
            child++;
            parent[x] = node;
            _Articulation(x, parent, discovery, low, path, v);
...
void Articulation(ll vertex, vector<ll> &path, vector<ll> v[])// the function to do DFS traversal
{
    ...
    for(ll i=0; i<vertex; i++) {
        if( discovery[i]==-1 ) {
            _Articulation(i, parent, discovery, low, path, v);// calls the recursive helper function to find articulation points in DFS tree rooted with vertex 'i'
        }
    }
}

bool uPath(ll ux, ll vx, vector<ll> path)
{
    ll j = vx;
    while( j != -1 ) {
        if( j==ux )
...
//    inout
    ...
    vector<ll> path(vertex,-1);// stores unique path
    Articulation(vertex, path, v);
    ll ux, vx;
    cin >> ux >> vx;
    cout << boolalpha << uPath(ux, vx, path) << lne;//returns true if there is an unique path between ux and vx
