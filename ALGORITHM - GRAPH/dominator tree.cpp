...
int sdom[Max],dom[Max],par[Max],dsu[Max],label[Max],arr[Max],dep[Max],rev[Max],T;
int ed[Max][2],ok[Max];
int n,m;
...
void Clear()
{
	for(int i = 1; i <= n; i++)
	{
		arr[i] = dep[i] = sdom[i] = dom[i] = par[i] = dsu[i] = label[i] = rev[i] = 0;
		tree[i].clear();
		g[i].clear();
		rg[i].clear();
		bucket[i].clear();
